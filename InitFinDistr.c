...
void distribute_nrgs(double *res, double *nrgspect_fin, double num_stars, double *num_sys_fin, double *num_bin_fin,
                     double num_clust) {
    unsigned int i;
    ...
    f_bin = 0.0;
    for (i = 0; i < SMPLPNTS_lE; i++) f_bin += res[2 * i + 1];
    f_bin *= BINWIDTH_lE;

    num_sys = num_stars / (1. + f_bin); /* number of systems (S+B) follows from number of stars in one cluster and
                                           binary fraction (eq. 13 in MK11) */
    ...
    *num_bin_fin += num_bin; /* cumulative number of binaries  */
    ...
    for (i = 0; i < SMPLPNTS_lE; i++) nrgspect_fin[2 * i + 1] += res[2 * i + 1] * BINWIDTH_lE * num_sys;
}
...
bool populate_initial_energy_distribution(char *libname, double *lib_entries, double *fE_in) {
    ...
    *lib_entries = 0.;
    ...
    fprintf(stdout, "Reading library...");
    fflush(stdout);
...
bool orbital_parameter_distributions(char *libname, char *outname, double lib_entries, double *nrgspect_in,
                                     double *nrgspect_fin, double *fE_in, bool SpT[], bool bdf[], bool constrain[],
                                     double min[], double max[], int smplpnts[], double binwidth[], double mmin,
                                     double mmax, int num_sys_obs, bool slidebinfrac, bool init, bool fin) {
    FILE *lib;
    FILE *outfile;
    ...
    double num_sys_ST[n_ST], mtmp, lib_size, binfrac[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0},
                                             binfrac_alt[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
    char tmp[10];
    int dummy;
    double param[NBDF];
    char SpT_names[][9] = {"M", "K", "G", "F", "A", "B", "O", "c", "u"},
         BDF_names[][12] = {"e", "q", "la", "lP", "lE", "mu", "lL", "ls", "t", "phi"}, filename[100];
    double num_sing_ST[n_ST], num_bin_ST[n_ST], num_bin_moving[32],
        num_sys_moving[32]; /* from 0.1Msun to 150Msun (log m=-1 > 2.2) */

    if (init || fin) {
        for (i = 0; i < NBDF; i++) f[i] = dmatrix(n_ST, smplpnts[i]);
    }
    char *out_bin_name[100];
    sprintf(out_bin_name, "%s_bin.dat", outname);
    outfile = fopen(out_bin_name, "w");
    /*
       scale final number of binaries in each bin to library size and round; this is why:

       fE_in contains the number of binaries per bin for the number of binaries in the library for the initial
       distribution;

       nrgspect_fin and nrgspect_in contain the *calculated* final and initial numbers of binaries per bin based on
       embedded cluster masses; has nothing to do with the size of the library, therefore these numbers need to be
       scaled according to the library size.

       it may occur that nrgspect_fin / nrgspect_in > 1, i.e. more binaries in one bin of the final than of initial
       distribution, meaning there are hardened binaries!

       LIB_SCALE_FACTOR < 1 ensures there will be sufficient hard binaries from the library that fall into the high
       energy bin if hardened binaries occur.
    */
    ...
    for (i = 0; i < SMPLPNTS_lE; i++) {
        ...
        if (nrgspect_in[2 * i + 1] != 0.) {
            ...
            if (init) nrgspect_fin[2 * i + 1] = fE_in[i] * LIB_SCALE_FACTOR;
            // else
    ...
    fprintf(stdout, "--------------------\n");
    fflush(stdout);
    fprintf(stdout, "Calculating binary-fractions and preparing OPDs...");
    fflush(stdout);

    row = -1.0; /* row number in library of intial binaries */
    dummy = fscanf(lib, "%s\t%s\t%s\t%s\t\t%s\t%s\t%s\t%s\t%s\t%s\n", tmp, tmp, tmp, tmp, tmp, tmp, tmp, tmp, tmp,
                   tmp); /* read library header */
    while (!feof(lib)) {
        dummy = fscanf(lib, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &m1, &m2, &e, &la, &lP, &lE, &lL, &ls,
                       &t, &h);
        row++;
        ...
        for (i = 0; i < SMPLPNTS_lE; i++) {
            if (lE > (lE_MIN + i * BINWIDTH_lE) && lE <= (lE_MIN + (i + 1) * BINWIDTH_lE)) {
                if (nrgspect_fin[2 * i + 1] > 0.01) { /* if a binary in spectrum left > leave as binary */
                    ...
                    nrgspect_fin[2 * i + 1] -=
                        1.0;    /* reduce number of binaries in the respective final energy bin by 1 */
                    ...
                    /* if row in library > scaled lib_size it means that there is a binary in the library although all
                     * should already have been distributed > it has to be a hardened binary */
                    ...
                    if (fin && slidebinfrac) {
                        j = 0;
                        while (j < 32) {
                            if (log10(m1) > ((-1.0 + j * 0.1) - 0.1) && log10(m1) < ((-1.0 + j * 0.1) + 0.1)) break;
                            j++;
                        }
                        num_sys_moving[j]++;
                        num_bin_moving[j]++;
                    ...
                    /* } else { // hardened binary
                      ...
                      num_bin_ST[7]++;
                      num_bin_ST[ST]++;
                      // two stars become one binary > subtract 1 system and 1 single star; number of systems with SpT
                    of the primary remains the same num_sys_ST[7]--; num_sing_ST[7]--;
                      // one single star with SpT becomes secondary of the new binary; one system and one single star
                    with SpT disappear ST=check_spectral_type(m2); num_sys_ST[ST]--; num_sing_ST[ST]--;
                    ...
                    if (row < lib_size) {
                        ...
                        if (fin && slidebinfrac) {
                            j = 0;
                            while (j < 32) {
                                if (log10(m2) > ((-1.0 + j * 0.1) - 0.1) && log10(m2) < ((-1.0 + j * 0.1) + 0.1)) break;
                                j++;
                            }
                            num_sys_moving[j]++;
                        }
                ...
                if (passes_constraints(param, constrain, min, max)) {
                    ...
                    for (j = 0; j < NBDF; j++) {
                        if (bdf[j]) {
                            ...
                            if (SpT[7]) bin_data(f[j][7], param[j], min[j], binwidth[j], smplpnts[j]);
                            if (SpT[ST]) bin_data(f[j][ST], param[j], min[j], binwidth[j], smplpnts[j]);
                        }
                    }
                }
            }
    ...
    if (init || fin) {
        ...
        for (i = 0; i < NBDF; i++) {
            if (bdf[i]) { /* if orbital-parameter requested for output */
                ...
                for (j = 0; j < smplpnts[i]; j++) xval[j] = min[i] + ((2 * j + 1) * binwidth[i]) / 2.0;

                for (j = 0; j < n_ST; j++) {
                    if (SpT[j]) { /* if additionally spectral type is requested for output > write to file */

                        if (j == 8)
                            sprintf(filename, "%s%s_distr_u_%.2lf-%.2lf_%s.dat", outname, BDF_names[i], mmin, mmax,
                                    (init ? "in" : "fin"));
                        else
                            sprintf(filename, "%s%s_distr_%s_%s.dat", outname, BDF_names[i], SpT_names[j],
                                    (init ? "in" : "fin"));
                        binfrac_alt[j] =
                            write_output(filename, xval, f[i][j], num_sys_ST[j], num_sys_obs, binwidth[i], smplpnts[i]);
    ...
    if (init || fin) {
        for (i = 0; i < NBDF; i++) dmatrix_free(f[i], n_ST);
    }
...
bool passes_constraints(double param[], bool constrain[], double min[], double max[]) {
    ...
    for (i = 0; i < NBDF; i++)
        if (constrain[i] && (param[i] > max[i] || param[i] < min[i])) return false;

    return true;
}

void bin_data(double *f, double sort_param, double min, double binwidth,
              double smplpnts) {  //,double **fx_P,unsigned int bin
    unsigned int i;
...
double write_output(char filename[], double *f_x, double *f_y, double num_sys, int num_sys_obs, double binwidth,
                    unsigned int smplpnts) {
    FILE *dat;
