...
int make_binary(double m1, double r1[3], double v1[3], double m2, double r2[3], double v2[3], double a, double e,
                double phase);
...
void Library(FILE *dat, long int Nlib, double mmin, double mmax, bool eigen, bool ordered, double alpha0, double lac,
             double alpha1) {
    clock_t t1, t2;
    t1 = clock();
    ...
    fprintf(stdout, "Generating library...");
    fflush(stdout);
    ...
    */
    ...
    if (MLOW > 0.5) { /* single power-law IMF above 0.5 Msun */
        ...
        k2 = k1 + 1.0 / c2 *
                      (pow(MHIGH, c2) -
                       pow(0.5, c2)); /* if 2-part power-law add power-law segment [0.5,MHIGH] to coefficient */
    ...
    for (i = 0; i < 2 * Nlib; i++) {
        do {
            x = drand48();
            if (x < k1 / k2) /* always true if IMF is single power-law below 0.5 Msun; always false if IMF is single
                                power-law above 0.5 */
    ...
    for (i = 0; i < 2 * Nlib - 1; i++) {
        ...
        do {
            if (i + j < 2 * Nlib - 1) {
                ...
                if (marray[i + j] != 0 && q > min_allowed_mr(mstar1, ordered) &&
                    q > min_allowed_mr(marray[i + j], ordered)) {
                    mstar2 = marray[i + j];
                    marray[i + j] = 0.0;
        ...
        a = pow(GN * P * 86400 * P * 86400 * (mstar1 + mstar2) / (4 * PI * PI), 1. / 3.) / 149597870.691; /* au */
        if (log10(a) < (lac)) {
            e1 = pow(xxx, 1.0 / (1 + alpha0));
            e2 = e1;
        } else {
            e1 = pow(xxx, 1.0 / (1 + alpha1));
            e2 = e1;
        }
        ...
        fprintf(dat, "\n% .4lf\t% .4lf\t% .4lf\t% .4lf\t% .4lf\t% .3lf\t% .3lf\t% .4lf\t% .4lf\t% .4lf", mstar1, mstar2,
                e2, log10(a), lP, lE, lL, ls, (t / P), phase);
    }
...
double usinu(double u, double e) { return ((u - e * sin(u)) / (2.0 * PI)); }
