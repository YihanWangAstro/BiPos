...
bool Synthesize(char *libname, char *outname, double rho, double mecl, int t, double beta, double sfr, double meclmin,
                double rh, double mmin, double mmax, int num_sys_obs, bool use_mk12, bool field, bool clust,
                bool slidebinfrac, bool init, bool fin, bool SpT[], bool bdf[], bool constrain[], double bin_min[],
                double bin_max[], int smplpnts[], double binwidth[]) {
    unsigned int i;
    FILE *dat;
    ...
    if (init) {
        ...
        error = orbital_parameter_distributions(libname, outname, lib_entries, nrgspect_in, nrgspect_fin, fE_in, SpT,
                                                bdf, constrain, bin_min, bin_max, smplpnts, binwidth, mmin, mmax,
                                                num_sys_obs, false, init, false);
        if (error) return true;
    }
    ...
    if (field) {
        ...
        while (mecl < meclmax) {
            ...
            for (i = 0; i < SMPLPNTS_lE; i++) sigm[2 * i + 1] = sigmoid(sigm[2 * i], slope, Ecut, asymptote);

            /* multiply logistic function with initial energy distribution to obtain energy distribution after dynamical
             * processing */
        ...
        /* construct and output orbital-parameter distributions after dynamical processing and summing up all star
         * clusters */
        error = orbital_parameter_distributions(libname, outname, lib_entries, nrgspect_in, nrgspect_fin, fE_in, SpT,
                                                bdf, constrain, bin_min, bin_max, smplpnts, binwidth, mmin, mmax,
                                                num_sys_obs, slidebinfrac, false, fin);
        if (error) return true;
    ...
    if (clust) {
        ...
        /* Set y-values for logistic function */
        for (i = 0; i < SMPLPNTS_lE; i++) sigm[i * 2 + 1] = sigmoid(sigm[i * 2], slope, Ecut, asymptote);
