...
void show_help(bool lib, bool clust, bool field, bool init, bool slidebinfrac, bool fin, bool eigen, bool ordered,
               bool spt);
void set_std_values(long int *Nlib, char *libname, double *mmin, double *mmax, bool *eigen, bool *ordered, double *rho,
                    double *mecl, int *num_sys_obs, double *t, double *beta, double *sfr, double *meclmin, double *rh,
                    bool *init, bool *fin, double *bin_min, double *bin_max, int *smplpnts, double *binwidth,
                    double *alpha);
bool extract_constraints(char argv[], double *bin_min, double *bin_max, int *smplpnts, double *binwidth);

bool check_user_input(int argc, char *argv[], char *libname, long int *Nlib, double *mmin, double *mmax,
                      int *num_sys_obs, bool *eigen, bool *ordered, double *rho, double *mecl, double *t, double *beta,
                      double *sfr, double *meclmin, double *rh, bool *lib, bool *use_mk12, bool *field, bool *clust,
                      bool *slidebinfrac, bool *init, bool *fin, bool *SpT, bool *bdf, bool *constrain, double *bin_min,
                      double *bin_max, int *smplpnts, double *binwidth, double *alpha0, double* lac, double *alpha1, char* outname) {
    unsigned int i, j;
    double s, k_ML, meclmax, m, r;
    bool help = false, spt = false, error = false;
    char *spectral_names[9] = {"M", "K", "G", "F", "A", "B", "O", "integrated", "user"},
         *bdf_names[10] = {"eccentricity", "mass-ratio",       "semi-major-axis",     "period",       "energy",
                           "reduced mass", "angular momentum", "apparent separation", "orbital-time", "phase"};
    char string[100];

    set_std_values(Nlib, libname, mmin, mmax, eigen, ordered, rho, mecl, num_sys_obs, t, beta, sfr, meclmin, rh, init,
                   fin, bin_min, bin_max, smplpnts, binwidth, alpha0);
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strncmp(argv[i], "out=", 4) == 0) sprintf(outname, "%s", argv[i] + 4);
        ...
        if (strncmp(argv[i], "alpha0=", 7) == 0) *alpha0 = atof(argv[i] + 7);
        if (strncmp(argv[i], "lac=", 4) == 0) *lac = atof(argv[i] + 4);
        if (strncmp(argv[i], "alpha1=", 7) == 0) *alpha1 = atof(argv[i] + 7);
        ...
        if (strncmp(argv[i], "constrain=", 10) == 0) {
            switch (argv[i][10]) {
                case 'f':
                    constrain[9] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[9]), &(bin_max[9]), &(smplpnts[9]), &(binwidth[9]));
                    break;
                case 't':
                    constrain[8] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[8]), &(bin_max[8]), &(smplpnts[8]), &(binwidth[8]));
                    break;
                case 's':
                    constrain[7] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[7]), &(bin_max[7]), &(smplpnts[7]), &(binwidth[7]));
                    break;
                case 'L':
                    constrain[6] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[6]), &(bin_max[6]), &(smplpnts[6]), &(binwidth[6]));
                    break;
                case 'm':
                    constrain[5] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[5]), &(bin_max[5]), &(smplpnts[5]), &(binwidth[5]));
                    break;
                case 'E':
                    constrain[4] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[4]), &(bin_max[4]), &(smplpnts[4]), &(binwidth[4]));
                    break;
                case 'P':
                    constrain[3] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[3]), &(bin_max[3]), &(smplpnts[3]), &(binwidth[3]));
                    break;
                case 'a':
                    constrain[2] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[2]), &(bin_max[2]), &(smplpnts[2]), &(binwidth[2]));
                    break;
                case 'q':
                    constrain[1] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[1]), &(bin_max[1]), &(smplpnts[1]), &(binwidth[1]));
                    break;
                case 'e':
                    constrain[0] = true;
                    error =
                        extract_constraints(argv[i] + 12, &(bin_min[0]), &(bin_max[0]), &(smplpnts[0]), &(binwidth[0]));
                    break;
                default:
                    fprintf(stderr, "\n'%c' is not a valid OPD!\n\n", argv[i][10]);
                    return (true);
    ...
    if (!(*lib) && !(*clust) && !(*field) && !help) help = true;
    ...
    if (*lib) {
        if (*mmin < 0.08) {
            fprintf(stderr,
                    "ERROR: minimum mass below hydrogen-burning mass-limit (mmin<0.08 Msun) -> increase 'mmin'.\n");
            fflush(stderr);
            return (true);
        }
    ...
    if (*clust || *field) {
        ...
        if (*rho != STD_RHO && (*mecl != STD_MECL || *rh != STD_RH)) {
            fprintf(stderr,
                    "ERROR: Set either rho=... or a combination of mecl=... and rh=..., from which rho is calculated, "
    ...
    if (*clust) {
        ...
        if (*use_mk12)
            sprintf(string, "%g pc (using rh=0.1xMecl^0.13 from Marks & Kroupa 2012)", *rh);
        else
        ...
        for (i = 0; i < NBDF; i++) {
            if (constrain[i]) {
                fprintf(stdout, "\n\t%s in range [%g:%g] and %i bins", bdf_names[i], bin_min[i], bin_max[i],
                        smplpnts[i]);
                j++;
            }
        }
        ...
        if (*mecl > 1e6) {
            fprintf(stdout,
                    "CAUTION! YOU'RE LOOKING AT A STAR CLUSTER WITH INITIAL MASS EXCEEDING 1e+06 MSUN (YOUR CHOICE: "
                    "MECL=%g MSUN)\n",
                    *mecl);
            fprintf(stdout,
                    "NOTE THAT FOR CLUSTERS THIS MASSIVE A TOP-HEAVY IMF MIGHT OCCUR WHICH IS NOT INCLUDED IN THE "
                    "PRESENT VERSION OF BIPOS.\n");
            fprintf(stdout, "TAKE CARE WHEN INTERPRETING THE RESULTS OBTAINED USING BIPOS.\n");
            fprintf(stdout, "--------------------\n");
        }
        if (*rho > 3e6) {
            fprintf(stdout,
                    "CAUTION! YOU'RE LOOKING AT A STAR CLUSTER WITH INITIAL DENSITY EXCEEDING 3e+06 MSUN PC^-3 (RHO=%g "
                    "MSUN PC^-3)\n",
                    *rho);
            fprintf(stdout,
                    "THIS IS BEYOND THE MAXIMUM DENSITY OF THE NBODY COMPUTATIONS BIPOS HAS BEEN GAUGED WITH.\n");
            fprintf(stdout, "TAKE CARE WHEN INTERPRETING THE RESULTS OBTAINED USING BIPOS.\n");
            fprintf(stdout, "--------------------\n");
        }
    ...
    if (*field) {
        ...
        if (meclmax < (*meclmin)) {
            fprintf(stderr,
                    "ERROR: max. cluster mass (%g) < min. cluster mass (%g) --> possible solution: increase SFR or "
                    "reduce MECLMIN.\n",
                    meclmax, *meclmin);
            fflush(stderr);
            return (true);
        ...
        if (meclmax > 1e6) {
            fprintf(stdout,
                    "CAUTION! YOUR STAR CLUSTER POPULATION CONTAINS CLUSTERS WITH INITIAL MASS EXCEEDING 1e+06 MSUN "
                    "(SFR=%g -> MECLMAX=%g MSUN)\n",
                    *sfr, meclmax);
            fprintf(stdout,
                    "NOTE THAT FOR CLUSTERS THIS MASSIVE A TOP-HEAVY IMF MIGHT OCCUR WHICH IS NOT INCLUDED IN THE "
                    "PRESENT VERSION OF BIPOS.\n");
            fprintf(stdout,
                    "PROBABLY A NEGLIGIBLE EFFECT IF SUCH CLUSTERS ARE NOT THE DOMINANT DONATORS TO THE FIELD "
                    "POPULATION.\n");
        ...
        if (!(*use_mk12) && 3 * meclmax / (8 * PI * (*rh) * (*rh) * (*rh)) > 3e6) {
            fprintf(stdout,
                    "CAUTION! YOUR STAR CLUSTER POPULATION CONTAINS CLUSTERS WITH INITIAL DENSITIES EXCEEDING 3e+06 "
                    "MSUN PC^-3.\n");
            fprintf(stdout,
                    "THIS IS BEYOND THE MAXIMUM DENSITY OF THE NBODY COMPUTATIONS BIPOS HAS BEEN GAUGED WITH.\n");
            fprintf(stdout, "TAKE CARE WHEN INTERPRETING THE RESULTS OBTAINED USING BIPOS.\n");
            ...
            do {
                ...
                if (3 * m / (8 * PI * pow(r, 3)) > 3e6) {
                    fprintf(stdout,
                            "CAUTION! YOUR STAR CLUSTER POPULATION CONTAINS CLUSTERS WITH INITIAL DENSITIES EXCEEDING "
                            "3e+06 MSUN PC^-3.\n");
                    fprintf(
                        stdout,
                        "THIS IS BEYOND THE MAXIMUM DENSITY OF THE NBODY COMPUTATIONS BIPOS HAS BEEN GAUGED WITH.\n");
    ...
    if (*fin && (*clust || *field)) {
        ...
        if (SpT[3] || SpT[4] || SpT[5] || SpT[6] || SpT[7] || (SpT[8] && *mmax > 1.4)) {
            ...
            fprintf(stdout,
                    "NOTE THAT THE KROUPA (1995) BIRTH/INITIAL BINARY POPULATIONS WHERE FOUND FOR LATE-TYPE STARS\n");
...
void show_help(bool lib, bool clust, bool field, bool slidebinfrac, bool init, bool fin, bool eigen, bool ordered,
               bool spt) {
    ...
    if (clust) {
        ...
        fprintf(stdout,
                "./BiPoS clust [OPD=EPLaqemtihs] [SpT=OBAFGKMc] [+init] [-evolve] [mecl=x rh=x] "
                "[constrain=x,min,max,bin] [libname=x] [t=1|3|5]\n\n");
        fprintf(stdout, "\tExplanation ('*'=required, STD=standard value)\n");
        fprintf(stdout, "\tclust(*)\ttells the program to synthesize a star cluster population\n");
        fprintf(stdout,
                "\tOPD=EPLaqemstf\tspecify which orbital-parameter distributions are to be computed. (STD=s)\n");
        fprintf(
            stdout,
            "\t\t\t(E-Energy,P-Period,L-angular momentum,a-semimajor axis,q-Mass-ratio,e-eccentrity,m-reduced mass,\n");
        ...
        fprintf(stdout,
                "\trh=x\t\treplace x with the half-mass radius in pc\n\t\t\tOR set rh=0 to use rh=0.1xMecl^0.13 from "
                "Marks & Kroupa 2012 (STD=0)\n");
        fprintf(stdout,
                "\trho=x\t\treplace x with the initial density within the half-mass radius of the embedded cluster in "
                "Msun pc^-3 (STD=%g)\n",
                STD_RHO);
        fprintf(stdout,
                "\t\t\t(calculated from mecl and rh instead, if not set; rho alone determines resulting binary "
                "population)\n");
        fprintf(stdout, "\tlibname=x\treplace x with filename of library in folder Lib/ (STD=%s)\n", STD_LIBNAME);
        ...
        fprintf(stdout,
                "\tconstrain=x,min,max,bin\t\treplace x with orbital-parameter identifier,\n\t\t\t\t\treplace 'min' "
                "with the minimum value for this parameter (log values for EPLa),\n\t\t\t\t\treplace 'max' with the "
                "maximum value for this parameter (log values for EPLa),\n\t\t\t\t\treplace 'bins' with the number of "
                "equal-size bins between min and max\n\n");
        fprintf(stdout,
                "Use 'constrain=...' repeatedly to place constraints on more than one parameter simultaneously. See "
                "examples below.\n");
        fprintf(stdout, "--------------------\n");
        fprintf(stdout, "Examples (distinguish upper- and lower-case characters!):\n");
        fprintf(stdout,
                "- Synthesize the period- and mass-ratio-OPD for G- and M-type binaries with embedded cluster mass "
                "1000Msun and half-mass radius 0.5pc\n  (note that only the initial density of the embedded cluster as "
                "calculated from mass and radius is relevant for the outcome):\n");
        fprintf(stdout, "  ./BiPoS clust OPD=Pq SpT=GM mecl=1000 rh=0.5\n");
        fprintf(stdout,
                "- Synthesize the semi-major-axis-, mass-ratio- and eccentrity-OPD for F-,K-type and all binaries for "
                "a cluster of density 500 Msun pc^-3:\n");
        fprintf(stdout, "  ./BiPoS clust OPD=aqe SpT=FKc rho=500\n");
        fprintf(stdout,
                "- Synthesize the initial energy-OPD, distribute in 10 bins for log(E) in [0:2] for G-type and all "
                "binaries from Library 'Lib/MyLib.dat',\n  but not the evolved population, and use standard values for "
                "Mecl and rh to calculate the density rho, output binaries with mass-ratios q in [0.6:0.9] only:\n");
        fprintf(
            stdout,
            "  ./BiPoS field OPD=E SpT=Gc +init -evolve constrain=E,0,2,10 constrain=q,0.6,0.9,3 libname=MyLib.dat\n");
    }
    ...
    if (field) {
        ...
        fprintf(stdout, "--------------------\n");
        fprintf(stdout,
                "In order to synthesize the binary population of a galactic field use the following Syntax:\n\n");
        fprintf(stdout,
                "./BiPoS field [OPD=EPLaqem] [SpT=OBAFGKMc] [+init] [-field] [beta=x sfr=x meclmin=x rh=x] "
                "[constrain=x,min,max,bin] [libname=x]\n\n");
        fprintf(stdout, "\tExplanation ('*'=required, STD=standard value)\n");
        ...
        fprintf(stdout, "\t+fbspt\t\toutput binary fraction as a continuous function of SpT (STD=%s)\n",
                slidebinfrac ? ("ON") : ("OFF"));
        fprintf(stdout,
                "\tbeta=x\t\treplace x with the index (negative slope) of the embedded cluster mass-function (ECMF, "
                "M^-beta, STD=%g)\n",
                STD_BETA);
        ...
        fprintf(stdout,
                "\trh=x\t\treplace x with the typical cluster half-mass radius in pc\n\t\t\tOR set rh=0 to use "
                "rh=0.1xMecl^0.13 from Marks & Kroupa 2012 (STD)\n");
        fprintf(stdout, "\tlibname=x\treplace x with filename of library in folder Lib/ (STD=%s)\n", STD_LIBNAME);
        fprintf(stdout, "\tscale=x\t\treplace x with number of targets in survey searched for companions (STD=%i)\n",
                STD_TARGETS);
        fprintf(stdout, "--------------------\n");
        ...
        fprintf(
            stdout,
            "- Synthesize field period- and mass-ratio-OPD for G- and M-type binaries;\n  choose ECMF index 2.0, SFR "
            "of 0.01 Msun yr^-1, minimum star cluster mass of 1 Msun and typical half-mass radius of 0.5 pc:\n");
        fprintf(stdout, "  ./BiPoS field OPD=Pq SpT=GM beta=2.0 sfr=0.01 meclmin=1.0 rh=0.5\n");
        fprintf(stdout,
                "- Synthesize field semi-major-axis-, mass-ratio- and eccentrity-OPD for F-,K-type and all binaries;\n "
                " choose ECMF index 2.4, SFR of 10 Msun yr^-1, minimum star cluster mass of 5 Msun and typical "
                "half-mass radius of 0.2 pc:\n");
        fprintf(stdout, "  ./BiPoS field OPD=aqe SpT=FKc beta=2.4 sfr=10.0 meclmin=5.0 rh=0.2\n");
        fprintf(stdout,
                "- Synthesize initial energy-OPD for G-type and all binaries from Library 'Lib/MyLib.dat',\n  but not "
                "for the field, and use standard values for beta, sfr, meclmin and rh:\n");
        fprintf(stdout, "  ./BiPoS field OPD=E SpT=Gc +init -field libname=MyLib.dat\n");
        fprintf(stdout,
                "- Synthesize initial and final field OPD for apparent separations and mass-ratios for A-type stars "
                "from Library 'Lib/MyLib.dat';\n  binaries with mass-ratios between 0.15 and 1.0 only; distribute "
                "mass-ratios into four equal-size bins;\n  use standard values for beta, sfr, meclmin and rh:\n");
        fprintf(stdout, "  ./BiPoS field OPD=sq SpT=A +init libname=MyLib.dat constrain=q,0.15,1.0,4\n");
        fprintf(stdout,
                "- Synthesize final field eccentriciy OPD for solar-type stars which have mass-ratios q in [0.6:0.9]\n "
    ...
    if (spt) {
        ...
        fprintf(stdout,
                "m/Msun | %.1lf- %.1lf | %.1lf-%.1lf | %.1lf-%.1lf | %.1lf-%.1lf | %.1lf-%.1lf | %.1lf-%.1lf | "
                "%.2lf-%.1lf\n",
                LOWMASS_O, MHIGH, LOWMASS_B, LOWMASS_O, LOWMASS_A, LOWMASS_B, LOWMASS_F, LOWMASS_A, LOWMASS_G,
                LOWMASS_F, LOWMASS_K, LOWMASS_G, LOWMASS_M, LOWMASS_K);
        fprintf(stdout, "--------------------\n");
        fprintf(stdout,
                "These values can be modified in 'Settings.h'; requires re-compilation via 'make clean' and "
                "'make'.\nUsually not necessary since a user-defined mass range can be defined via the command-line "
                "during runtime.\n");
    }
}
